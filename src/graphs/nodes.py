...
**重要**：以上是用户明确提出想去的地点，必须安排在行程中，并作为核心亮点！"""
    ...
    planning_prompt = f"""⚠️⚠️⚠️ 严格时间约束警告 ⚠️⚠️⚠️
【以下规则必须100%遵守，违反任何一条都是严重错误！】

1. 🌅 日落/夕阳观景点：只能安排在【17:00-19:00】
   ❌ 错误：09:00 日落观景 ← 这是严重逻辑错误！
   ✅ 正确：17:30 日落观景

2. 🎢 主题公园（环球影城、迪士尼等）：必须安排【全天】
   ❌ 错误：17:00 环球影城 ← 3小时根本不够玩！
   ✅ 正确：09:00-21:00 环球影城（全天）

3. 🏨 酒店入住：第一天抵达时入住（08:00-10:00）
   ❌ 错误：主题公园后18:00入住 ← 玩一天太累了才办入住？
   ✅ 正确：08:00 抵达后先办理入住放行李

4. 🌙 夜景/灯光秀：只能在【19:00-22:00】

---

基于收集的信息，为用户制定详细且合理的{days}天{travel_style}行程。
...
### 住宿安排逻辑（最重要！）
1. **住宿是前一天晚上入住**：酒店入住应该安排在游玩某个区域的【前一天晚上】，而不是玩完之后
2. **第一天抵达**：如果第一天抵达，先入住酒店放行李，再开始轻松游玩
3. **全天主题公园（如环球影城、迪士尼）**：
   - 应在前一天晚上入住附近酒店
   - 当天只安排主题公园游玩（09:00-21:00），不要安排入住活动
   - 如果是第一天玩主题公园，则抵达后先入住酒店（安排在早上08:00），然后去玩
4. **不要在同一天既安排全天景点又安排入住**：入住要么在前一天，要么在抵达时

### 时间安排逻辑
5. **每天景点数**：2-3 个主要景点为宜，不要安排过多导致走马观花
6. **全天景点**：主题公园需要全天时间（09:00-21:00），当天不要安排其他主要景点

### 特定活动时间（非常重要！）
7. **日落/夕阳观景点**：必须安排在傍晚 17:00-19:00，绝对不能安排在早上！
8. **日出观赏**：必须安排在清晨 05:00-07:00
9. **夜景/灯光秀**：必须安排在晚上 19:00-22:00
10. **早市/早餐体验**：安排在早上 07:00-09:00

### 行程合理性
11. **用户必去地点优先**：{', '.join(must_visit_places) if must_visit_places else '无特殊要求'} 必须安排在行程中！
12. **地理位置优化**：同一天的景点应该在同一区域，避免来回折腾
13. **交通时间**：活动之间预留 30-60 分钟交通时间
14. **用餐时间**：12:00 左右午餐，18:00 左右晚餐

### 类型限制
15. **type 类型**：只能是 attraction（景点）、food（餐饮）、hotel（住宿）、transport（交通）
