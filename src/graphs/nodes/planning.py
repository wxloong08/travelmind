    ...
    for day in itinerary:
        ...
        for activity in day.get("activities", []):
            ...
            # === 餐饮：继承上一个位置 + 小偏移 ===
                ...
                if prev_location:
                    activity["location"] = {
                        "lat": prev_location["lat"] + 0.0008,  # ~80m 偏移
                        "lng": prev_location["lng"] + 0.0008,
                    }
...
async def planning_node(state: AgentState) -> dict[str, Any]:
    ...
    restaurant_names = _build_poi_names_list(poi_pool, category="餐厅")
...
请尝试：调整景点顺序、替换 1-2 个景点、选择不同区域酒店"""
    ...
    try:
        ...
        try:
            import json_repair
            structured_plan = json_repair.repair_json(content, return_objects=True)
        except Exception:
    ...
    itinerary, coord_warnings = _populate_coordinates(
        itinerary, 
        poi_pool,
        arrival_hub,
    )
